...
class TestCertificatesRouter:
    ...
    @patch("routers.certificates.certificate_manager.get_cert_info")
    def test_certificate_info_endpoint(self, mock_get_cert_info, auth_client):
    ...
    def test_certificate_issue_requires_auth(self, client):
        """Test that the certificate issue endpoint requires authentication."""
        response = client.post("/api/v1/certificates/issue", json={})
        # Accept either 401 or 403 as valid authentication failure
        assert response.status_code in [401, 403]
    
    @patch("routers.certificates.certificate_manager.issue_certificate")
    def test_certificate_issue_endpoint(self, mock_issue_certificate, auth_client):
    ...
    def test_certificate_renew_requires_auth(self, client):
        """Test that the certificate renew endpoint requires authentication."""
        response = client.post("/api/v1/certificates/renew", json={})
        # Accept either 401 or 403 as valid authentication failure
        assert response.status_code in [401, 403]
    
    @patch("routers.certificates.certificate_manager.check_cert_status")
    @patch("routers.certificates.certificate_manager.renew_certificates")
    def test_certificate_renew_endpoint(self, mock_renew_certificates, mock_check_cert_status, auth_client):
        ...
        response = auth_client.post("/api/v1/certificates/renew", json={"force_renewal": True})
        assert response.status_code == 200
    ...
    def test_certificate_status_requires_auth(self, client):
    ...
    @patch("routers.certificates.certificate_manager.check_cert_status")
    @patch("routers.certificates.certificate_manager.get_cert_expiry_date")
    def test_certificate_status_endpoint(self, mock_get_cert_expiry_date, mock_check_cert_status, auth_client):
        """Test the certificate status endpoint."""
        # Mock certificate status
        mock_check_cert_status.return_value = (True, 25)  # Has certs, 25 days remaining
        mock_get_cert_expiry_date.return_value = "2023-08-01T00:00:00"
        
        # Set the renew_before_days value
        certificate_manager._renew_before_days = 30
        certificate_manager._cert_dir = "./certs"
        certificate_manager._challenge_type = "http-01"
        certificate_manager._auto_renew = True
